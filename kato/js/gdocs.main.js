...
$(window).load(function() {
	...
	$('#find').click(function(event){
		var umaName = $('#uma_name').val();
		var umaWeather = $('#uma_weather').val();
		var umaTime = $('#uma_time').val();

		var conditions = [];

		if (umaName != ''){
			conditions.push({'name':'name', 'value':umaName, 'hitEmpty':false});
		}
		if (umaWeather != ''){
			conditions.push({'name':'weather', 'value':umaWeather, 'hitEmpty':true});
		}
		if (umaTime != ''){
			conditions.push({'name':'time', 'value':umaTime, 'hitEmpty':true});
		}

		find_from_conditions($('#ep').val(), conditions, viewLocationResult, 100, 'ja', $('#lat').val(), $('#lng').val());
	...
	$('#uma_time').val(getCurrentTimeZone());
...
var viewWeatherResult = function(d, lat, lng){
	...
	$('#uma_weather').val(getCurrentWeatherFromWeatherDesc(condition.weatherDesc[0].value));
};
